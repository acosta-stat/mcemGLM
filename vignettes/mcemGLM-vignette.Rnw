...
\item Steps 2 and 3 are repeated until the condition
  ...
  The default values in the package are $\delta = 0.05$ and $\epsilon = 0.01$ but these can be easily changed by the user. The default number of iterations is 150 and this value can also be changed by the user.
...
We can also use this matrix to get predictions of the observed random effects.
<<echo=TRUE>>=

...
gradientLogit_t <- function(pars, u, sigmaType, kKi, kLh, kLhi, kY, kX, kZ) {
  ...
  loglikelihoodSigma <- function(pars, u) {
    ovSigma <- constructSigma_n(pars = pars[-1], sigmaType = sigmaType, kK = kK, kR = kR, kLh = kLh, kLhi = kLhi)
    return(logMarginalCpp_t(sigma = ovSigma, df = pars[1], sigmaType = sigmaType, u = u, kKi = kKi, kLh = kLh, kLhi = kLhi))
  }

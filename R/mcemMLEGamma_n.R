...
mcemMLEGamma_n <- function(sigmaType, kKi, kLh, kLhi, kY, kX, kZ, initial, controlEM, controlTrust) {
  ...
  if (controlEM$MCsd == 0) {
    ...
    while (ar > 0.4 | ar < 0.15) {
      uSample <- uSamplerGammaCpp_n(beta = beta, sigma = ovSigma, alpha = alpha, u = u, kY = kY, kX = kX, kZ = kZ, B = 2000, sd0 = sdtune)
  ...
  while (j <= controlEM$EMit & sum(tail(errorCounter, 3)) < 3) {
    ...
    if (ar < 0.15 | ar > 0.4) {
      ...
      while (ar > 0.4 | ar < 0.15) {
        uSample.tmp <- uSamplerGammaCpp_n(beta = beta, sigma = ovSigma, alpha = alpha, u = u, kY = kY, kX = kX, kZ = kZ, B = 2000, sd0 = sdtune)
  ...
  iMatrix <- iMatrixDiagGammaCpp_n(beta = beta, sigma = ovSigma, alpha = alpha, uSample = uSample, kKi = kKi, kY = kY, kX = kX, kZ = kZ, B = controlEM$MCit, sd0 = controlEM$MCsd)

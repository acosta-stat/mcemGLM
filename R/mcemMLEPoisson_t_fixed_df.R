...
mcemMLEPoisson_t_fixed_df <- function (sigmaType, df, kKi, kLh, kLhi, kY, kX, kZ, initial, controlEM, controlTrust) {  
  ...
  while (j <= controlEM$EMit & sum(tail(errorCounter, 3)) < 3) {
    ...
    if (j > 30 & controlEM$speedup == TRUE) {
      ...
      iJ <- iJacobDiagPoissonCpp_t(beta = beta, sigma = ovSigma, sigmaType = sigmaType, 
                                   uSample = uSample, df = df, kKi = kKi, kLh = kLh, kLhi = kLhi, 
                                   kY = kY, kX = kX, kZ = kZ, B = controlEM$MCit, sd0 = controlEM$MCsd)

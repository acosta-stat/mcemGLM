...
mcemMLENegBinom_t_fixed_df <- function(sigmaType, df, kKi, kLh, kLhi, kY, kX, kZ,
  ...
  if (controlEM$MCsd == 0) {
    ...
    while (ar > 0.4 | ar < 0.2) {
      uSample <- uSamplerNegBinomCpp_t(beta = beta, sigma = ovSigma, alpha = alpha,
  ...
  while (j <= controlEM$EMit & sum(tail(errorCounter, 3)) < 3) {
    ...
    # Now we optimize.
    outTrust <- trust(toMaxDiagNegBinom_t, parinit = theta, rinit = controlTrust$rinit,
    ...
    if (ar < 0.2 | ar > 0.4) {
      ...
      u <- rnorm(kK, rep(0, kK), sqrt(diag(ovSigma))) # Initial value for u
      while (ar > 0.4 | ar < 0.2) {
        uSample.tmp <- uSamplerNegBinomCpp_t(beta = beta, sigma = ovSigma,
                                             alpha = alpha, sigmaType = sigmaType,
                                             u = u, df = df, kKi = kKi, kLh = kLh,
                                             kLhi = kLhi, kY = kY, kX = kX, kZ = kZ,
                                             B = 5000, sd0 = sdtune)
  ...
  iMatrix <- iMatrixDiagNegBinomCpp_t(beta = beta, sigma = ovSigma, alpha = alpha,
                                      sigmaType = sigmaType, uSample = uSample, 
                                      df = df, kKi = kKi, kLh = kLh, kLhi = kLhi, 
                                      kY = kY, kX = kX, kZ = kZ, B = B0, 
                                      sd0 = controlEM$MCsd)

  colnames(uSample) <- colnames(kZ)
  
  # loglikehood MCMC
  QfunMCMC <- MCMCloglikelihoodNegBinomCpp_t(beta = beta, sigma = ovSigma,
                                             alpha = alpha, sigmaType = sigmaType,
                                             u = uSample, df = df, kKi = kKi,
                                             kLh = kLh, kLhi = kLhi, kY = kY,
                                             kX = kX, kZ = kZ)
  
  # The return statement is like this because ifelse can't return objects of different sizes.

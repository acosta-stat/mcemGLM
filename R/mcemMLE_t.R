...
mcemMLE_t <- function (sigmaType, sigmaDim, kKi, kLh, kLhi, kY, kX, kZ, EMit, MCit, MCf) {
  ...
  for (j in 2:EMit) {
    ...
    # Now we optimize.
    outOptim <- optim(par = theta, fn = toMax_t, control = list(fnscale = -1, maxit = 10000), u = uSample, sigmaType = sigmaType, sigmaDim = sigmaDim, kKi = kKi, kLh = kLh, kLhi = kLhi, kY = kY, kX = kX, kZ = kZ)
